...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** PEXA START */
    case OP_PEXA_ASSET              : return "OP_PEXA_ASSET";
    /** PEXA END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_PEXA_ASSET) { // OP_PEXA_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == PEXA_R) { // Check to see if PEXA starts at 27 ( this->size() < 105)
                if ((*this)[28] == PEXA_V)
                    if ((*this)[29] == PEXA_N)
                        index = 30;
            } else {
                if ((*this)[28] == PEXA_R) // Check to see if PEXA starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == PEXA_V)
                        if ((*this)[30] == PEXA_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == PEXA_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == PEXA_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == PEXA_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == PEXA_R) {
