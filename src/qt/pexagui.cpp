...
const QString PexaGUI::DEFAULT_WALLET = "~Default";

PexaGUI::PexaGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void PexaGUI::createActions()
{
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to PEXA addresses"));
    ...
    openAction->setStatusTip(tr("Open a pexa: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Pexa command-line options").arg(tr(PACKAGE_NAME)));
...
void PexaGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/pexacointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* pexaLabelLayout = new QVBoxLayout(toolbarWidget);
        pexaLabelLayout->addWidget(label);
        pexaLabelLayout->addWidget(toolbar);
        pexaLabelLayout->setDirection(QBoxLayout::TopToBottom);
        pexaLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Pexa Coin Market Price"));
...
#ifdef ENABLE_WALLET
bool PexaGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool PexaGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void PexaGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void PexaGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void PexaGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Pexa"); // default title
...
#ifdef ENABLE_WALLET
void PexaGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    ...
    if (assetName == "PEXA")
        ...
        msg += tr("Amount: %1\n").arg(PexaUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void PexaGUI::setHDStatus(int hdEnabled)
{
...
static bool ThreadSafeMessageBox(PexaGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void PexaGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    {
        max_width = qMax(max_width, fm.width(PexaUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (PexaUnits::Unit u : PexaUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(PexaUnits::name(u)), this);
...
{
    setText(PexaUnits::name(newUnits));
}
...
void PexaGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=PEXABTC"));
