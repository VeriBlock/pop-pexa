...
#include <qt/forms/ui_faststart.h>
...
void FastStart::DownloadSnapshotUrl() 
{
    ...
    auto snapshotUrl = Params().SnapshotUrl();
    auto url_url = QString::fromStdString(gArgs.GetArg("-snapshoturl", snapshotUrl));
...
{
    if(reply->error()) {
        ui->statusLabel->setText(ErrorText(tr(reply->errorString()))); 
...
void FastStart::ExtractSnapshot()
{
    ...
    QString snapshot_path = dest + "latest.zip";
