...
void ReissueAssetDialog::CheckFormState()
{
    ...
    if (asset->nAmount + ui->quantitySpinBox->value() * COIN > MAX_MONEY) {
        showMessage(tr("Quantity is to large. Max is 17,122,989"));
        return;
    }
    ...
    if (!ui->addressText->text().isEmpty()) {
        if (!IsValidDestination(dest)) {
            showMessage(tr("Invalid Pexa Destination Address"));
            return;
        }
    }
...
void ReissueAssetDialog::onAssetSelected(int index)
{
    ...
    if (index > 0) {
        ...
        ui->quantitySpinBox->setMaximum(17122989 - value.get_real());
...
void ReissueAssetDialog::onReissueAssetClicked()
{
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetReissueAssetBurnAmount(), 8)) + " PEXA";
    amount.append("</b>");
