...
#include <qt/pexaaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Pexa address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
#endif
    widget->setValidator(new PexaAddressEntryValidator(parent));
    widget->setCheckValidator(new PexaAddressCheckValidator(parent));
}
...
bool parsePexaURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no pexa: URI
    if(!uri.isValid() || uri.scheme() != QString("pexa"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!PexaUnits::parse(PexaUnits::PEXA, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsePexaURI(QString uri, SendCoinsRecipient *out)
{
    // Convert pexa:// to pexa:
    //
    //    Cannot handle this later, because pexa:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("pexa://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "pexa:");
    }
    QUrl uriInstance(uri);
    return parsePexaURI(uriInstance, out);
}

QString formatPexaURI(const SendCoinsRecipient &info)
{
    QString ret = QString("pexa:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(PexaUnits::format(PexaUnits::PEXA, info.amount, false, PexaUnits::separatorNever));
        paramCount++;
    }
...
bool openPexaConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(PEXA_CONF_FILENAME);
    ...
    /* Open pexa.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Pexa.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Pexa (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Pexa (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "pexa.desktop";
    return GetAutostartDir() / strprintf("pexa-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a pexa.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Pexa\n";
        else
            optionFile << strprintf("Name=Pexa (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the pexa app
...
bool GetStartOnSystemStartup()
{
    CFURLRef pexaAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (pexaAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, pexaAppUrl);

    CFRelease(pexaAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add pexa app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, pexaAppUrl, nullptr, nullptr);
    }
