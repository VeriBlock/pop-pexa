...
static const std::regex PEXA_NAMES("^PEXA$|^PEXA$|^PEXACOIN$");
...
bool IsRootNameValid(const std::string& name)
{
    return std::regex_match(name, ROOT_NAME_CHARACTERS)
        ...
        && !std::regex_match(name, PEXA_NAMES);
}
...
void CNewAsset::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(PEXA_R); // r
    vchMessage.push_back(PEXA_V); // v
    vchMessage.push_back(PEXA_N); // n
    vchMessage.push_back(PEXA_Q); // q

    vchMessage.insert(vchMessage.end(), ssAsset.begin(), ssAsset.end());
    script << OP_PEXA_ASSET << ToByteVector(vchMessage) << OP_DROP;
}
...
{
    // Must contain at least 3 outpoints (PEXA burn, owner change and one or more new unique assets that share a root (should be in trailing position))
    if (vout.size() < 3) {
...
{
    // Reissuing an Asset must contain at least 3 CTxOut ( Pexa Burn Tx, Any Number of other Outputs ..., Reissue Asset Tx, Owner Asset Change Tx)
    if (vout.size() < 3) {
...
{
    // Check the first transaction and verify that the correct PEXA Amount
...
{
    // Verify 'pexaq' is in the transaction
...
{
    // Verify 'pexar' is in the transaction
    CScript scriptPubKey = txOut.scriptPubKey;
...
bool CreateAssetTransaction(CWallet* pwallet, CCoinControl& coinControl, const std::vector<CNewAsset> assets, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    if (!change_address.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Pexa address: ") + change_address);
            return false;
        }
    } else {
    ...
    // Check to make sure the wallet has the PEXA required by the burnAmount
...
bool CreateReissueAssetTransaction(CWallet* pwallet, CCoinControl& coinControl, const CReissueAsset& reissueAsset, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    if (!IsValidDestinationString(address)) {
        error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Pexa address: ") + address);
        return false;
    }
...
bool CreateTransferAssetTransaction(CWallet* pwallet, const CCoinControl& coinControl, const std::vector< std::pair<CAssetTransfer, std::string> >vTransfers, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    CAmount curBalance = pwallet->GetBalance();
    if (curBalance == 0) {
        error = std::make_pair(RPC_WALLET_INSUFFICIENT_FUNDS, std::string("This wallet doesn't contain any PEXA, transfering an asset requires a network fee"));
        return false;
    }
