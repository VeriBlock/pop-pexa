    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    auto* pexatip = pop.altTree->btc().getBestChain().tip();
    LogPrintf("%s: new best=ALT:%d:%s %s %s version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)%s\n", __func__,
        ...
        (pexatip ? pexatip->toShortPrettyString() : "BTC:nullptr"),
        pindexNew->nVersion,
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    {
        if (block.GetBlockTime() > nAdjustedTime + MAX_FUTURE_BLOCK_TIME_DGW || block.GetBlockTime() > nAdjustedTime + VeriBlock::GetPop().config->alt->maxFutureBlockTime())
