...
# TestP2PConn: A peer we use to send messages to pexad, and store responses.
...
class CompactBlocksTest(PexaTestFramework):
    ...
    def build_block_on_tip(self, node, segwit=False):
        ...
        block = create_block(node, int(tip, 16), create_coinbase(height + 1), mtp + 1)
        block.nVersion = 4
        if segwit:
    ...
    # This test actually causes pexad to (reasonably!) disconnect us, so do this last.
    ...
    # Test that pexad requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for pexad to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # pexad will not send blocktxn responses for blocks whose height is
