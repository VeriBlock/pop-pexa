...
class BIP65Test(PexaTestFramework):
    ...
    def run_test(self):
        ...
        block = create_block(self.nodes[0], int(tip, 16), create_coinbase(CLTV_HEIGHT - 1), block_time)
        block.nVersion = 3
        ...
        tip = block.sha256
        block_time += 1
        block = create_block(self.nodes[0], tip, create_coinbase(CLTV_HEIGHT), block_time)
        block.nVersion = 3
        block.solve()
